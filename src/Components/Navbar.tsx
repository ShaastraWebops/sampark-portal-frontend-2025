...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`flex justify-between w-full h-28 font-bold bg-[#0a1128] sticky top-0 opacity-80 z-10 transition-transform duration-300 ${showNavbar ? '' : 'transform -translate-y-full'}`}>
      <div className="h-full pt-2 w-2/12">
        <img src={logo} className="mt-1 h-full w-5/6" alt="Shaastra Logo" />
      </div>
      <div className="-mt-2 text-3xl w-3/5 flex justify-between items-center h-full mr-12 text-slate-200">
